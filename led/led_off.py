#!/usr/bin/env python3
"""
LED OFF - Standalone script for shutdown
Turns off Sonoff LED devices at system shutdown
"""
...
async def turn_off_led():
    """Turn off the first online LED device"""
    ...
    try:
        ...
        if online_switches:
            ...
            logging.info(f"Sending OFF command to {device['name']}")
...
def main():
    """Main entry point"""
    log_file = r"E:\Scripts\led\led_off.log"
    setup_logging(log_file)
    
    logging.info("=" * 50)
    logging.info("LED OFF script started")
    logging.info("=" * 50)
    
    try:
        success = asyncio.run(turn_off_led())
