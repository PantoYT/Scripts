...
Usage: python shutdown_manager.py [--no-confirm] [--cancel-usb-sync]
"""
...
class ShutdownManager:
    ...
    def stop_usb_sync(self) -> bool:
        ...
        if killed_any:
            ...
            self.logger.info("  No sync processes running")
            print("  No sync processes running")
    ...
    def close_applications(self) -> bool:
        ...
        self.logger.info("[STEP 4/5] Gracefully closing applications...")
        print("\n" + "=" * 60)
        print("[STEP 4/5] Closing applications...")
        print("=" * 60)
        
        # Applications to close gracefully (without /F force flag)
        graceful_apps = [
            "chrome.exe",
            "firefox.exe",
            "msedge.exe",
        ...
        print("  Force closing remaining applications...")
        for app in graceful_apps:
            subprocess.run(
                ["taskkill", "/F", "/IM", app],
                capture_output=True,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Safe System Shutdown Manager"
    )
    parser.add_argument(
        "--no-confirm",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-led",
        action="store_true",
        help="Skip LED shutdown"
    )
    parser.add_argument(
        "--delay",
        type=int,
        default=10,
        help="Shutdown delay in seconds (default: 10)"
    )
